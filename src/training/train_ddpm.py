...
def save_logs(run_dir, loss_buffer, step, model, optimizer, scheduler, device, image_shape, latent_shape, images_mean, images_std, vae, num_samples=9, checkpoint=True):
...
def train_ddpm(model, dataloader, scheduler, train_config, device, image_shape, latent_shape, images_mean, images_std, vae): # scheduler, dataloader on cpu; model on device
    ...
    while step < train_config.max_steps:
        for images, labels in dataloader:
            ...
            if step % train_config.early_checkpoint_every == 0 and step < train_config.num_early_checkpoints * train_config.early_checkpoint_every:
                save_logs(train_config.run_dir, loss_buffer, step, model, optimizer, scheduler, device, image_shape, latent_shape, images_mean, images_std, vae, checkpoint=False)
            elif step % train_config.late_checkpoint_every == 0 and step > 0:
                save_logs(train_config.run_dir, loss_buffer, step, model, optimizer, scheduler, device, image_shape, latent_shape, images_mean, images_std, vae)
