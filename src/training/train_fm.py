...
def save_logs(run_dir, loss_buffer, step, model, optimizer, fm_config, device, image_shape, images_mean, images_std, num_samples=100):
    ...
    samples = sample(model, x0, c, cfg_scale, fm_config) # history list from x0 to xT; (T, N, C, H, W)
...
def train_fm(model, dataloader, fm_config, train_config, device, image_shape, images_mean, images_std): # dataloader on cpu; model on device
    ...
    while step < train_config.max_steps:
        for images, labels in dataloader:
            ...
            if step % train_config.early_checkpoint_every == 0 and step < train_config.num_early_checkpoints * train_config.early_checkpoint_every:
                save_logs(train_config.run_dir, loss_buffer, step, model, optimizer, fm_config, device, image_shape, images_mean, images_std)
