...
def euler_step(x, v, num_steps): # x: (B, C, H, W), v: (B, C, H, W), num_steps: int
    step_size = torch.full((x.shape[0], 1, 1, 1), 1.0 / num_steps, dtype=torch.float64, device=x.device)
...
def sample(model, x0, fm_config): # x0: (B, C, H, W)
    ...
    for t in range(fm_config.num_steps):
        t_batch = torch.full((x.shape[0],), 1.0 * t / fm_config.num_steps, dtype=torch.float64, device=x.device)
        with torch.no_grad():
            v = model(x, t_batch)
